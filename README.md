...
[H]: 海水是咸的主要原因是因为其中含有大量的溶解盐类，其中最主要的是氯化钠（NaCl）。氯化钠是一种非常常见的盐类，它存在于地球上的大部分地区，包括海洋、湖泊、河流和地下水。当水从海洋中蒸发时，它会带走一部分盐分，使海水变得咸。此外，海中还含有其他溶解盐类的物质，如硫酸盐、硫酸镁、硫酸钙、硫酸镁盐、硫酸镁钾、硫酸镁钠、硫酸镁钙、硫酸镁镁钙、硫酸镁镁钙、硫酸镁镁钙、硫酸镁镁钠、硫酸镁镁钙、硫酸镁镁镁钙、硫酸镁镁镁钙、硫酸镁镁镁钙、硫酸镁镁镁钙、硫酸镁镁镁钙、硫酸镁镁镁钙、硫酸镁镁镁钙、硫酸镁镁镁钙、硫酸镁镁镁钙、硫酸镁镁镁钙、硫酸镁镁镁镁钙、硫酸镁镁镁镁钙、硫酸镁镁镁镁钙、硫酸镁镁镁镁钙、硫酸镁镁镁镁钙、硫酸镁镁镁镁镁钙、硫酸镁镁镁镁镁钙、硫酸镁镁镁镁镁钙、硫酸镁镁镁镁镁钙、硫酸镁镁镁镁镁镁钙、硫酸镁镁镁镁镁镁钙、硫酸镁镁镁镁镁镁钙、硫酸镁镁镁镁
...
| 操作类型 | 命令 | 说明 |
|---------|------|------|
| 克隆项目 | `git clone https://github.com/jingyaogong/minimind.git` | 下载项目代码 |
| 安装依赖 | `pip install -r requirements.txt -i https://mirrors.aliyun.com/pypi/simple` | 安装Python依赖包 |
| 下载模型 | `git clone https://huggingface.co/jingyaogong/MiniMind2` | 下载预训练模型 |

### 二、模型训练命令

| 训练类型 | 单卡训练命令 | 多卡训练命令 | 输出文件 |
|---------|-------------|-------------|---------|
| 预训练 | `python train_pretrain.py` | `torchrun --nproc_per_node N train_pretrain.py` | `pretrain_*.pth` |
| 全参数SFT | `python train_full_sft.py` | `torchrun --nproc_per_node N train_full_sft.py` | `full_sft_*.pth` |
| LoRA微调 | `python train_lora.py` | `torchrun --nproc_per_node N train_lora.py` | `lora_xxx_*.pth` |
| DPO训练 | `python train_dpo.py` | `torchrun --nproc_per_node N train_dpo.py` | `dpo_*.pth` |
| PPO训练 | `python train_ppo.py` | `torchrun --nproc_per_node N train_ppo.py` | `ppo_actor_*.pth` |
| GRPO训练 | `python train_grpo.py` | `torchrun --nproc_per_node N train_grpo.py` | `grpo_*.pth` |
| SPO训练 | `python train_spo.py` | `torchrun --nproc_per_node N train_spo.py` | `spo_*.pth` |
| 推理模型训练 | `python train_reason.py` | `torchrun --nproc_per_node N train_reason.py` | `reason_*.pth` |

### 三、模型测试与推理

| 操作类型 | 命令 | 说明 |
|---------|------|------|
| 命令行测试 | `python eval_llm.py --weight full_sft` | 测试指定权重模型 |
| WebUI启动 | `streamlit run web_demo.py` | 启动Web界面（需在scripts目录） |
| 断点续训 | `python train_pretrain.py --from_resume 1` | 从检查点恢复训练 |
| WandB记录 | `python train_xxx.py --use_wandb` | 启用训练过程记录 |

### 四、第三方推理框架

| 框架 | 启动命令 | 说明 |
|------|----------|------|
| Ollama | `ollama run jingyaogong/minimind2` | 本地运行模型 |
| vLLM | `vllm serve ./MiniMind2/ --served-model-name "minimind" --port 8998` | 高性能推理服务 |
| llama.cpp | `./build/bin/llama-cli -m ../minimind/MiniMind2/MiniMind2.gguf` | C++推理引擎 |

### 五、API服务

| 服务类型 | 启动命令 | 说明 |
|---------|----------|------|
| OpenAI兼容API | `python serve_openai_api.py` | 启动API服务端 |
| API客户端测试 | `python chat_openai_api.py` | 测试API接口 |
| cURL测试 | `curl http://ip:port/v1/chat/completions -H "Content-Type: application/json" -d '{"model": "model-identifier", "messages": [{"role": "user", "content": "问题"}]}'` | HTTP接口测试 |

### 六、模型转换

| 转换类型 | 命令 | 说明 |
|---------|------|------|
| Torch转Transformers | `python convert_model.py` | 格式转换 |
| HF转GGUF | `python convert_hf_to_gguf.py ../minimind/MiniMind2` | llama.cpp格式转换 |
| 模型量化 | `./build/bin/llama-quantize ../minimind/MiniMind2/MiniMind2.gguf ../minimind/MiniMind2/Q4-MiniMind2.gguf Q4_K_M` | 模型压缩 |

### 七、常用参数说明

| 参数 | 含义 | 示例 |
|------|------|------|
| `--weight` | 指定权重名称 | `full_sft`, `pretrain`, `dpo`等 |
| `--load_from` | 模型加载路径 | `model`（原生torch权重） |
| `--save_dir` | 权重保存目录 | 默认`out`目录 |
| `--lora_weight` | LoRA权重名称 | `lora_medical`等 |
| `--historys` | 历史对话轮数 | 偶数，0表示不携带历史 |
| `--max_new_tokens` | 最大生成长度 | 默认8192 |
| `--temperature` | 生成温度 | 默认0.85 |
| `--top_p` | nucleus采样阈值 | 默认0.85 |
...
- **单机多卡训练**：使用`torchrun --nproc_per_node N`命令，N为GPU数量
- **断点续训**：所有训练脚本支持`--from_resume 1`参数自动恢复训练
- **权重保存**：默认每100步保存一次，文件保存在`./out/`目录
- **可视化**：支持WandB/SwanLab训练过程记录

> 注意：所有训练命令需要在项目根目录或相应子目录下执行，具体路径请参考原文档说明。
