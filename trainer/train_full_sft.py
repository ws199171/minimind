...
监督微调阶段使用高质量的对话数据，通过标准的语言建模目标训练模型。
与预训练不同，SFT阶段：
- 只训练SFT数据格式的对话内容
- 使用较低的学习率和较小的batch size
- 只计算assistant回复部分的loss

全参数微调 vs LoRA微调：
- 全参数微调：更新所有模型参数
- LoRA微调：只更新低秩适配器参数，原始模型冻结
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", type=float, default=1e-6, help="初始学习率（SFT通常较小）")
    ...
    parser.add_argument('--from_weight', default='pretrain', type=str, help="基于哪个权重训练（通常为pretrain）")
